...
1. Navigate to the repo directory.
1. `cmake -S . -B fwbuild`
1. `cmake --build fwbuild`
1. Write the firmware depending on which board you are using:
    * `./deploy.sh /path/to/device fwbuild/firmware/pico/firmware_pico_rp2040.uf2`
    * `./deploy.sh /path/to/device fwbuild/firmware/qtpy/firmware_qtpy_rp2040.uf2`
    * `./deploy.sh /path/to/device fwbuild/firmware/promicro/firmware_promicro.uf2`
    * `./deploy.sh /path/to/device fwbuild/firmware/tiny/firmware_tiny2040.uf2`
    * `./deploy.sh /path/to/device fwbuild/firmware/trinkey/firmware_trinkey2040.uf2`
...
|   |___/common                     // Code common to all versions
|
|___/examples                       // Bus Host Demo apps
|   |___cpu_chart_matrix_ltp305.py  // CPU utilization display for twin 5x7 LTP305 matrix LEDs
|   |___cpu_chart_matrix.py         // CPU utilization display for 8x8 HT16K33-based matrix LEDs
|   |___cpu_chart_segment.py        // CPU utilization display for 4-digit, 7-segment LEDs
|   |___mcp9808_temp.py             // Periodic temperature reports from an MCP9808 sensor
|
...
Under macOS, RP2040-based boards will appear in `/dev` as `cu.usbmodemXXXXX` or `cu.usbserialXXXXXX`. You can use my [`dlist()`](https://blog.smittytone.net/2022/09/08/how-to-manage-serial-devices-on-mac/) Z-shell function to save looking up and keying in these names, which can vary across boots.
...
`cli2c` is a command line driver for the USB-connected RP2040-based I&sup2;C host, which must be pre-loaded with the firmware.

It is a generic I&sup2;C driver with the following syntax:
...
* `device_port` is the USB-connected Bus Host’s Unix device path, eg. `/dev/cu.usbmodem-101`.
...
| :-: | :-: | --- |
| `z` |  | Initialise the target I&sup2;C bus. The bus is not initialised at startup |
| `c` | `{bus}` `{sda_pin}` `{scl_pin}` | Choose the host’s I&sup2;C bus (0 or 1) and the SDA and SCL pins by their RP2040 GPIO number. Defaults are board-specific and shown below. You must select a bus before initialising it |
| `f` | {frequency} | The I&sup2;C bus frequency in multiples of 100kHz. Supported values: 1 and 4 |
| `w` | `{address}` `{data_bytes}` | Write the supplied data to the I&sup2;C device at `address`. `data_bytes` are comma-separated 8-bit hex values, eg. `0x4A,0x5C,0xFF` |
| `r` | `{address}` `{count}` | Read `count` bytes from the I&sup2;C device at `address` and issue an I&sup2;C STOP |
| `p` |  | Issue an I&sup2;C STOP. Usually used after one or more writes |
| `x` |  | Reset the I&sup2;C bus |
| `s` |  |  Display devices on the I&sup2;C bus. **Note** This will initialise the bus if it is not already initialised |
| `i` |  |  Display I&sup2;C host device information |
| `g` | {pin_number} [hi|lo] [in|out] | [Set a GPIO pin](#gpio). |
| `l` | {`on`\|`off`} | Turn the Bus Host LED on or off |
| `h` |  |  Display help information |

#### Error and Data Output

All message output is routed via `stderr`. All data read back from an I&sup2;C device is output to `stdout` so it can be captured to a file. Returned data is currently presented as hexadecimal strings. For example:
...
| Board | I&sup2;C Bus | SDA Pin | SCL Pin |
| --- | :-: | :-: | :-: |
| Pico | 1 | 2 | 3 |
| QTpy&#42; | 1 | 22 (26) | 23 (27) |
| Trinkey | 0 | 16 | 17 |
...
Devices marked with &#42; have a STEMMA-QT port as well as pins. The values in brackets are the bus and pins used when the STEMMA-QT is set to be the default. This can be done uncommenting the line:

```
set(USE_STEMMA 1)
```

in the board’s `CMakeLists.txt` file.

But don’t forget that you can also use the `c` command to choose an alternative I&sup2;C bus and pins.
...
```shell
matrix {device} [I&sup2;C address] [commands]
```

**Note** Again, arguments in braces `{}` are required; those in square brackets `\[\]` are optional.

* `{device}` is the path to the Bus Host device file, eg. `/dev/cu.usbserial-DO029IEZ`.
* `[I&sup2;C address]` is an optional I&sup2;C address. By default, the HT16K33 uses the address `0x70`, but this can be changed.
...
Version 1.2.0 also supports [Pimoroni’s LED Dot Matrix Breakout boards](https://shop.pimoroni.com/products/led-dot-matrix-breakout?variant=32274405621843). These use the I&sup2;C address range 0x61-0x63, so you can select which matrix type your commands are applied to by specifying an I&sup2;C address in this range:
...
```shell
segment {device} [I&sup2;C address] [commands]
```

**Note** Again, arguments in braces `{}` are required; those in square brackets `\[\]` are optional.

* `{device}` is the path to the Bus Host’s device file, eg. `/dev/cu.usbserial-DO029IEZ`.
* `[I&sup2;C address]` is an optional I&sup2;C address. By default, the HT16K33 uses the address `0x70`, but this can be changed.
* `[commands]` are a sequence of command blocks as described below.
...
My own I&sup2;C driver code started out based on James’ but involves numerous changes and (I think) improvements. I also removed the non-macOS code removed and some unneeded functionality that I don’t need (I&sup2;C capture, monitoring). Finally, it targets fresh firmware I wrote from the ground up to run on an RP2040-based board, not the I2CMini.
